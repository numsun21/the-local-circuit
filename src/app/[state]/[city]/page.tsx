...
export default async function CityPage({ params }: { params: Promise<{ state: string, city: string }> }) {
  ...
  const { data: articles } = await supabase.from('articles').select('*').ilike('city', city).order('published_at', { ascending: false })
  ...
  return (
    <div style={{ fontFamily: 'Georgia, serif', background: '#fff', minHeight: '100vh' }}>
      ...
      <div style={{ maxWidth: '1100px', margin: '0 auto', padding: '3rem 2rem', display: 'grid', gridTemplateColumns: '1fr 300px', gap: '4rem' }}>
        <div>
          <p style={{ fontSize: '11px', textTransform: 'uppercase', letterSpacing: '0.12em', color: '#999', marginBottom: '0.5rem' }}>{stateName} · Local Coverage</p>
          <h1 style={{ fontSize: '56px', fontWeight: 900, letterSpacing: '-0.02em', marginBottom: '0.25rem' }}>{cityName}</h1>
          <p style={{ fontStyle: 'italic', color: '#888', fontSize: '16px', marginBottom: '2rem', paddingBottom: '2rem', borderBottom: '3px double #e0e0e0' }}>Student journalism from {cityName}, {stateName}</p>

          {(!articles || articles.length === 0) ? (
            <div style={{ textAlign: 'center', padding: '4rem 0', color: '#ccc' }}>
              <p style={{ fontStyle: 'italic', fontSize: '20px', marginBottom: '1.5rem' }}>No stories from {cityName} yet.</p>
        ...
        <aside>
          <div style={{ position: 'sticky', top: '2rem' }}>
            <p style={{ fontSize: '11px', textTransform: 'uppercase', letterSpacing: '0.12em', color: '#999', marginBottom: '1rem', paddingBottom: '0.5rem', borderBottom: '0.5px solid #e0e0e0' }}>Other Cities</p>
            {['Columbus', 'Cleveland', 'Cincinnati', 'Toledo', 'Akron', 'Dayton'].filter(c => c.toLowerCase() !== city).map(c => (
              <a key={c} href={`/${state}/${c.toLowerCase()}`} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '0.6rem 0', borderBottom: '0.5px solid #f0f0f0', textDecoration: 'none', color: 'inherit' }}>
            ...
            <div style={{ marginTop: '1.5rem', padding: '1.25rem', background: '#f9f9f7', border: '0.5px solid #e8e8e4' }}>
              <p style={{ fontSize: '14px', fontWeight: 700, marginBottom: '0.4rem' }}>Cover {cityName}</p>
              <p style={{ fontSize: '12px', color: '#888', marginBottom: '1rem', lineHeight: 1.6 }}>We're looking for student writers in {cityName}.</p>
