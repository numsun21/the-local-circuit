...
export default function AdminPage() {
  ...
  const handleLogin = (e: React.FormEvent) => {
    e.preventDefault()
    if (password === ADMIN_PASSWORD) { setAuthed(true); fetchSubmissions() }
  ...
  if (selected) return (
    <main style={{ fontFamily: 'Georgia, serif', maxWidth: '800px', margin: '0 auto', padding: '2rem 1.5rem' }}>
      <button onClick={() => setSelected(null)} style={{ fontSize: '12px', textTransform: 'uppercase', letterSpacing: '0.08em', color: '#666', background: 'none', border: 'none', cursor: 'pointer', marginBottom: '2rem' }}>← Back</button>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '1rem' }}>
        ...
        <button
          ...
          disabled={publishing || selected.published}
          style={{ padding: '12px 24px', background: selected.published ? '#ccc' : '#111', color: '#fff', border: 'none', borderRadius: '8px', fontSize: '13px', textTransform: 'uppercase', letterSpacing: '0.08em', cursor: selected.published ? 'default' : 'pointer', whiteSpace: 'nowrap' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column' }}>
          {submissions.map(s => (
            <div key={s.id} onClick={() => setSelected(s)} style={{ padding: '1.25rem 0', borderBottom: '0.5px solid #ddd', cursor: 'pointer', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              ...
              {s.published && <span style={{ fontSize: '11px', textTransform: 'uppercase', letterSpacing: '0.08em', color: '#999', border: '0.5px solid #ccc', padding: '4px 10px', borderRadius: '20px' }}>Published</span>}
            </div>
          ))}
        </div>
      )}
